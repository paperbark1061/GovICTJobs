                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ForEach(roles, id: \.self) { role in
                            Toggle(role, isOn: Binding(
                                ...
                                set: { isSelected in
                                    if isSelected { filters.roles.insert(role) }
                                    else { filters.roles.remove(role) }
                                }
                            ))
                        }
                    }
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ForEach(locations, id: \.self) { location in
                            Toggle(location, isOn: Binding(
                                ...
                                set: { isSelected in
                                    if isSelected { filters.locations.insert(location) }
                                    else { filters.locations.remove(location) }
                                }
                            ))
                        }
                    }
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ForEach(arrangements, id: \.self) { arrangement in
                            Toggle(arrangement, isOn: Binding(
                                ...
                                set: { isSelected in
                                    if isSelected { filters.arrangements.insert(arrangement) }
                                    else { filters.arrangements.remove(arrangement) }
                                }
                            ))
                        }
                    }
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ForEach(modules, id: \.self) { module in
                            Toggle(module, isOn: Binding(
                                ...
                                set: { isSelected in
                                    if isSelected { filters.modules.insert(module) }
                                    else { filters.modules.remove(module) }
                                }
                            ))
                        }
                    }
